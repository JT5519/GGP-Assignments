...
void Game::Init()
{
	...
	//  - Others, like setting shaders, will need to be moved elsewhere later
	{
		...
		//Ask D3D to create constant buffer in GPU memory.
		{
		...
		//binding the register 0 to the constant buffer.
...
void Game::CreateGeometry()
{
	...
	//Creating Meshes
	{
		...
		// Hexagon
		{
			Vertex vertices[] =
			{
				{ XMFLOAT3(+0.0f, +0.0f, +0.0f), gold },
				{ XMFLOAT3(+0.5f, +0.0f, +0.0f), blue },
				{ XMFLOAT3(+0.25f, +0.433f, +0.0f), blue },
				{ XMFLOAT3(-0.25f, +0.433f, +0.0f), blue },
				{ XMFLOAT3(-0.5f, +0.0f, +0.0f), blue },
				{ XMFLOAT3(-0.25f, -0.433f, +0.0f), blue },
				{ XMFLOAT3(+0.25f, -0.433f, +0.0f), blue },
	...
	//Creating Entities
	{
		//Triangle Entity 1
		{
			gameEntities.push_back(std::make_shared<Entity>(gameMeshes[0]));
		}
		...
		//Hexagon Entity 1
		{
			gameEntities.push_back(std::make_shared<Entity>(gameMeshes[1]));
		}
		...
		//Square Entity
		{
			gameEntities.push_back(std::make_shared<Entity>(gameMeshes[2]));
		}
	}
}
...
void Game::Draw(float deltaTime, float totalTime)
{
	...
	for (std::shared_ptr<Entity> entity : gameEntities)
	{
		...
		vsData.projectionMatrix = mainCamera->GetProjectionMatrix();

		D3D11_MAPPED_SUBRESOURCE mappedBuffer = {};
		context->Map(vsConstantBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD,
			0, &mappedBuffer);
