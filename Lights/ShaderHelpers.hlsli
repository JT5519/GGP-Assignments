...
float Specular(float3 dirToView, float3 dirReflected, float roughness)
{
    float specExponent = clamp((1.0f - roughness) * MAX_SPECULAR_EXPONENT, 0.05f, MAX_SPECULAR_EXPONENT); //clamp so that for perfect matte materials, exp is not 0
    return pow(saturate(dot(dirToView, dirReflected)), specExponent);
}

float3 Directional(Light light, float3 normal, float3 viewPosition, float3 worldPosition, float roughness, float3 surfaceColor)
{
    //diffuse
    ...
    float specular = Specular(dirToView, dirOfReflection, roughness);
    
    //diffuse + specular
    return (diffuse * surfaceColor + specular) * light.Color * light.Intensity;
}

float3 Point(Light light, float3 normal, float3 viewPosition, float3 worldPosition, float roughness, float3 surfaceColor)
{
